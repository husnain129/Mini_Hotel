...
app.get("/api/v1/restaurants/:id/reviews", async (req, res) => {
    try {
        const id = req.params.id;
        const result = await db.query("SELECT r.name AS restaurant_name,re.name,re.review,re.rating FROM restaurants r INNER  JOIN reviews re ON re.restaurant_id = r.id WHERE r.id=$1", [
            id,
        ]);
